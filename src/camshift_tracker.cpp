...
cv::Mat CamshiftTracker::getHistogramImage()
{
	...
	for (int i = 0; i < hsize_; i++) {
		int val = cv::saturate_cast<int>(hist_.at<float>(i)*hist_img.rows / 255);
...
void CamshiftTracker::tracking(TrackObject& trk_obj, cv::RotatedRect& trackBox)
{
	...
	if (FirstTracking(trk_obj.state)) {
		...
		cv::calcHist(&roi, 1, 0, mask_roi, hist_, 1, &hsize_, &phranges_);
	...
	cv::Mat backproj;
	cv::calcBackProject(&hue_, 1, 0, hist_, backproj, &phranges_);
	backproj &= mask_;
